...
Original file is located at
    https://colab.research.google.com/drive/1tf-kS9aj8GGSlcqQagIZBXc04XJ0gZRX
...
for items in events_matrix: 
  if len(items) > 1: # Removing single note events
    it = []
...
for note in output_song:
 if note[1] < 256 and note[2] < 256: 
  ...
  output_string += '='
  output_string += str(note[6].decode('ascii', 'replace')).replace('\\', '').replace('/', '').replace(' ', '')
  output_string += '\n'
...
input_prompt = "I love you" #@param {type:"string"}
...
TMIDI.Tegridy_SONG_to_MIDI_Converter(song, output_file_name=fname, output_signature='Karaoke-MASTER', track_name=song_name)
